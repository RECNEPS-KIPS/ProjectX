    ...
    public class UIBindingInspector : Editor {
        ...
        private bool IsValid {
            get {
                ...
                return !string.IsNullOrEmpty(_pathSerializedProperty.stringValue) && File.Exists(Path.Combine(Application.dataPath, $"Scripts/UI/{_pathSerializedProperty.stringValue}.cs"));
            }
        }
        ...
        private void OnEnable() {
            ...
            if (File.Exists(path)) {
                ...
                var className = Regex.Match(content, @"class\s+[a-zA-Z_][0-9a-zA-Z_]*\s*:\s*BaseWindow\s*\{").Value.Replace("class","").Replace("BaseWindow","").Replace(":","").Replace("{","").Replace(" ","");
        ...
        private void Check() {
            ...
            var cacheBindMap = _uiBinding.BinderDataList.ToDictionary(data => data.bindKey, data => new BinderData {
                bindKey = data.bindKey, bindFieldId = data.bindFieldId, bindObj = data.bindObj,fieldType = data.bindFieldId % DEF.BIND_ENUM_GAP,
            });
            //查字段
            if (string.IsNullOrEmpty(_pathSerializedProperty.stringValue)) return;
            var path = Path.Combine(Application.dataPath, $"Scripts/UI/{_pathSerializedProperty.stringValue}.cs");
