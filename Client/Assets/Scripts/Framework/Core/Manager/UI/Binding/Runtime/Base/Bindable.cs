        ...
        private void UpdateBind(dynamic value) {
            ...
            switch ((LinkerType)data.fieldType) {
                ...
                case LinkerType.Quaternion:
                    if (value is Quaternion quaternionValue) baseBinder.SetQuaternion(data.bindObj, data.bindFieldId,quaternionValue);
                    break;
                ...
                case LinkerType.UIBinding:
                    break;
                default:
                    LogManager.LogWarning("Failure Binding",$"Unregistered binding type : {((LinkerType)data.fieldType).ToString()}");
                    break;
            }
        }
    }
