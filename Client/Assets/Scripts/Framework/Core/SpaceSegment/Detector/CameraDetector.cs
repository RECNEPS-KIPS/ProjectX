...
namespace Framework.Core.SpaceSegment
{
    ...
    /// 该触发器根据相机裁剪区域触发
    /// </summary>
    public class CameraDetector : DetectorBase
    {
        ...
        {
            return MathUtils.ComputeOutCode(position, matrix);
        }
#if UNITY_EDITOR
        void OnDrawGizmos()
        {
            ...
            if (camera)
            {
                GizmosUtils.DrawViewFrustum(camera, Color.yellow);
            }
        }
#endif
    }
