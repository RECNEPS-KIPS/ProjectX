...
namespace ZZZ
{
    public class CharacterComboBase
    {
        ...
        public CharacterComboBase(Animator animator, Transform playerTransform, Transform cameraTransform,
            ...
            PlayerEnemyDetectionData playerEnemyDetectionData, Player player)
        {
        ...
        public void PlayComboFX()
        {
            SFX_PoolManager.MainInstance.TryGetSoundPool(
                reusableData.currentCombo.GetComboSoundStyle(reusableData.currentIndex.Value), playerTransform.position,
                Quaternion.identity);
        }
        ...
        protected bool AttackDetection(ComboContainerData comboContainerData)
        {
            ...
            if (DevelopmentToos.GetAngleForTargetDirection(GameBlackboard.MainInstance.GetEnemy(), playerTransform) <
                80)
            {
                return false;
            }
        ...
        private void AttackTrigger()
        {
            if (animator.AnimationAtTag("ATK"))
            {
                ...
                CameraHitFeel.MainInstance.CameraShake(
                    reusableData.currentCombo.GetComboShakeForce(reusableData.currentIndex.Value,
                        reusableData.ATKIndex));
                ...
                CameraHitFeel.MainInstance.PF(
                    reusableData.currentCombo.GetPauseFrameTime(reusableData.currentIndex.Value,
                        reusableData.ATKIndex));

                #region

                if (reusableData.canQTE && reusableData.ATKIndex >=
                    reusableData.currentCombo.GetComboATKCount(reusableData.currentIndex.Value))
                {
                    TriggerSwitchSkill();
                }
            ...
            else if (animator.AnimationAtTag("Skill"))
            {
                ...
                GameEventsManager.MainInstance.CallEvent("", reusableData.currentSkill.comboDamage,
                    reusableData.currentSkill.hitName, reusableData.currentSkill.parryName, playerTransform,
                    GameBlackboard.MainInstance.GetEnemy(), this);

                #region

                if (reusableData.currentSkill.pauseFrameTimeList != null &&
                    reusableData.currentSkill.pauseFrameTimeList.Length > 0 && reusableData.ATKIndex <=
                    reusableData.currentSkill.pauseFrameTimeList.Length)
                {
                    CameraHitFeel.MainInstance.PF(
                        reusableData.currentSkill.pauseFrameTimeList[reusableData.ATKIndex - 1]);
                }
                else
                {
                ...
                if (reusableData.currentSkill.shakeForce == null ||
                    reusableData.ATKIndex > reusableData.currentSkill.shakeForce.Length)
                {
                    return;
                }
            ...
            else //
            {
                ...
                GameEventsManager.MainInstance.CallEvent("",
                    comboData.executeCombo.GetComboDamage(reusableData.executeIndex));
            }
        }
        ...
        public void UpdateAttackLookAtEnemy()
        {
            ...
            {
                if (DevelopmentToos.DistanceForTarget(playerTransform, GameBlackboard.MainInstance.GetEnemy()) >
                    6.5f) return;
                if (DevelopmentToos.DistanceForTarget(playerTransform, GameBlackboard.MainInstance.GetEnemy()) <
                    0.09f) return;
        ...
        {
            SFX_PoolManager.MainInstance.TryGetSoundPool(SoundStyle.ComboVoice, comboData.comboName,
                playerTransform.position);
        }

        protected void PlayWeaponSound(ComboData comboData)
        {
            SFX_PoolManager.MainInstance.TryGetSoundPool(SoundStyle.WeaponSound, comboData.comboName,
                playerTransform.position);
        }
