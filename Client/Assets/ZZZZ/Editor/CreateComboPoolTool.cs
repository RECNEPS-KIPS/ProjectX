...
public class CreateComboPoolTool : MonoBehaviour
{
        ...
        foreach (string guid in guids)
        {
...

            {
                if (comboData.weaponSound != null)
                {
                    ...
                    {
                        CreatePrefabAndAddToPool(comboData, serializedSoundPool, serializedPoolProperty,
                            SoundStyle.WeaponSound);
                    }
                }
    ...
    private static void CreatePrefabAndAddToPool(ComboData comboData, SerializedObject serializedSoundPool,
        SerializedProperty serializedPoolProperty, SoundStyle soundStyle)
    {
        ...
        for (int i = 0; i < serializedPoolProperty.arraySize; i++)
        {
            ...
            if (poolItem.FindPropertyRelative("soundName").stringValue == soundItem.soundName &&
                poolItem.FindPropertyRelative("soundStyle").enumValueIndex == (int)soundItem.soundStyle)
            {
                if (!found)
                {
